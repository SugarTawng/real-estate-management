...
const LandSaleList = require("../models/LandSaleList");
...
module.exports = {
    ...
    try {
      ...
      LandSaleList.findOne({
        ...
        if (result) {
          ...
          return callback(1, "invalid_LandSaleList", 403, null, null);
        }
      });
    } catch (error) {
      return callback(1, "get_one_LandSaleList_fail", 400, error, null);
    }
  },
  ...
  update: function (
    ...
    try {
      ...
      queryObj.manager_sale_id = updateData.manager_sale_id;
      ...
      LandSaleList.update(queryObj, { where: where })
        .then((result) => {
          "use strict";
  ...
  delete: function (accessUserId, accessUserType, id, callback) {
    try {
      ...
      LandSaleList.findOne({ where: where })
        .then((LandSaleList) => {
          "use strict";
          if (LandSaleList && LandSaleList.deleted === Constant.DELETED.YES) {
            LandSaleList.destroy({ where: where })
              .then((result) => {
                return callback(null, null, 200, null);
              })
              .catch(function (error) {
                return callback(1, "remove_LandSaleList_fail", 420, error);
              });
          } else {
            LandSaleList.update(queryObj, { where: where })
              .then((result) => {
              ...
              .catch(function (error) {
                return callback(1, "update_LandSaleList_fail", 420, error);
              });
          }
        })
        .catch(function (error) {
          "use strict";
          return callback(1, "find_one_LandSaleList_fail", 400, error, null);
        });
    } catch (error) {
      return callback(1, "delete_LandSaleList_fail", 400, error);
    }
  },
    ...
    try {
      ...
      LandSaleList.findOne({
        ...
        .then((LandSaleList) => {
          "use strict";
          if (LandSaleList) {
            if (LandSaleList.activated === "false") {
              return callback(1, "unactivated_user", 404, null, null);
            } else {
              BCrypt.compare(
                password,
                LandSaleList.password,
                function (error, result) {
                  if (result === true) {
                    return callback(null, null, 200, null, LandSaleList);
                  } else {
  ...
  createByAdmin: function (
    ...
    try {
      ...
      console.log('aaaaaaaaaaaaaaaa toi da o day');

      let queryObj = {};

      
      queryObj.sale_id = userData.sale_id;
      queryObj.manager_sale_id = userData.manager_sale_id;
