...
const LandListOwner = require("../models/LandListOwner");
...
module.exports = {
    ...
    try {
      ...
      LandListOwner.findOne({
        ...
        if (result) {
          ...
          return callback(1, "invalid_LandListOwner", 403, null, null);
        }
      });
    } catch (error) {
      return callback(1, "get_one_LandListOwner_fail", 400, error, null);
    }
  },
  ...
  update: function (
    ...
    try {
      ...
      LandListOwner.update(queryObj, { where: where })
        .then((result) => {
          "use strict";
  ...
  delete: function (accessUserId, accessUserType, id, callback) {
    try {
      ...
      LandListOwner.findOne({ where: where })
        .then((LandListOwner) => {
          "use strict";
          if (LandListOwner && LandListOwner.deleted === Constant.DELETED.YES) {
            LandListOwner.destroy({ where: where })
              .then((result) => {
                return callback(null, null, 200, null);
              })
              .catch(function (error) {
                return callback(1, "remove_LandListOwner_fail", 420, error);
              });
          } else {
            LandListOwner.update(queryObj, { where: where })
              .then((result) => {
              ...
              .catch(function (error) {
                return callback(1, "update_LandListOwner_fail", 420, error);
              });
          }
        })
        .catch(function (error) {
          "use strict";
          return callback(1, "find_one_LandListOwner_fail", 400, error, null);
        });
    } catch (error) {
      return callback(1, "delete_LandListOwner_fail", 400, error);
    }
  },
