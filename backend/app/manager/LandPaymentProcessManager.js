...
const LandPaymentProcess = require("../models/LandPaymentProcess");
...
module.exports = {
    ...
    try {
      ...
      LandPaymentProcess.findOne({
        ...
        if (result) {
          ...
          return callback(1, "invalid_LandPaymentProcess", 403, null, null);
        }
      });
    } catch (error) {
      return callback(1, "get_one_LandPaymentProcess_fail", 400, error, null);
    }
  },
  ...
  getAll: function (
    ...
    try {
      ...
      LandPaymentProcess.findAndCountAll({
        where: where,
  ...
  //             .then((data) => {
  //                 let pages = Math.ceil(data.count / perPage);
  //                 let LandPaymentProcesss = data.rows;
  //                 let output = {
  //                     data: LandPaymentProcesss,
  //                     pages: {
  //                         current: page,
  //                         prev: page - 1,
  //                         hasPrev: false,
  ...
  update: function (
    ...
    try {
      ...
      queryObj.land_area_id = updateData.land_area_id;
      queryObj.payment_time = updateData.payment_time;
      queryObj.amount_of_money = updateData.amount_of_money;
      queryObj.amount_of_debt = updateData.amount_of_debt;
      queryObj.submitter = updateData.submitter;
      queryObj.status = updateData.status;
      queryObj.updated_at = new Date();

      LandPaymentProcess.update(queryObj, { where: where })
        .then((result) => {
          "use strict";
  ...
  delete: function (accessUserId, accessUserType, id, callback) {
    try {
      ...
      LandPaymentProcess.findOne({ where: where })
        .then((LandPaymentProcess) => {
          "use strict";
          if (LandPaymentProcess && LandPaymentProcess.deleted === Constant.DELETED.YES) {
            LandPaymentProcess.destroy({ where: where })
              .then((result) => {
                return callback(null, null, 200, null);
              })
              .catch(function (error) {
                return callback(1, "remove_LandPaymentProcess_fail", 420, error);
              });
          } else {
            LandPaymentProcess.update(queryObj, { where: where })
              .then((result) => {
              ...
              .catch(function (error) {
                return callback(1, "update_LandPaymentProcess_fail", 420, error);
              });
          }
        })
        .catch(function (error) {
          "use strict";
          return callback(1, "find_one_LandPaymentProcess_fail", 400, error, null);
        });
    } catch (error) {
      return callback(1, "delete_LandPaymentProcess_fail", 400, error);
    }
  },
  ...
  authenticate: function (loginName, password, callback) {
    try {
      ...
      LandPaymentProcess.findOne({
        ...
        .then((LandPaymentProcess) => {
          "use strict";
          if (LandPaymentProcess) {
            if (LandPaymentProcess.activated === "false") {
              return callback(1, "unactivated_user", 404, null, null);
            } else {
              BCrypt.compare(
                password,
                LandPaymentProcess.password,
                function (error, result) {
                  if (result === true) {
                    return callback(null, null, 200, null, LandPaymentProcess);
                  } else {
  ...
  createByAdmin: function (
    ...
    try {
      ...
      queryObj.payment_time = userData.payment_time;
      queryObj.amount_of_money = userData.amount_of_money;
      queryObj.amount_of_debt = userData.amount_of_debt;
      queryObj.submitter = userData.submitter;
      ...
      LandPaymentProcess.create(queryObj)
        .then((result) => {
          "use strict";
          return callback(null, null, 200, null, result);
        })
