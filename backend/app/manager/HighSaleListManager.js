...
const HighSaleList = require("../models/HighSaleList");
...
module.exports = {
    ...
    try {
      ...
      HighSaleList.findOne({
        ...
        if (result) {
          ...
          return callback(1, "invalid_HighSaleList", 403, null, null);
        }
      });
    } catch (error) {
      return callback(1, "get_one_HighSaleList_fail", 400, error, null);
    }
  },
  ...
  update: function (
    ...
    try {
      ...
      HighSaleList.update(queryObj, { where: where })
        .then((result) => {
          "use strict";
  ...
  delete: function (accessUserId, accessUserType, id, callback) {
    try {
      ...
      HighSaleList.findOne({ where: where })
        .then((HighSaleList) => {
          "use strict";
          if (HighSaleList && HighSaleList.deleted === Constant.DELETED.YES) {
            HighSaleList.destroy({ where: where })
              .then((result) => {
                return callback(null, null, 200, null);
              })
              .catch(function (error) {
                return callback(1, "remove_HighSaleList_fail", 420, error);
              });
          } else {
            HighSaleList.update(queryObj, { where: where })
              .then((result) => {
              ...
              .catch(function (error) {
                return callback(1, "update_HighSaleList_fail", 420, error);
              });
          }
        })
        .catch(function (error) {
          "use strict";
          return callback(1, "find_one_HighSaleList_fail", 400, error, null);
        });
    } catch (error) {
      return callback(1, "delete_HighSaleList_fail", 400, error);
    }
  },
    ...
    try {
      ...
      HighSaleList.findOne({
        ...
        .then((HighSaleList) => {
          "use strict";
          if (HighSaleList) {
            if (HighSaleList.activated === "false") {
              return callback(1, "unactivated_user", 404, null, null);
            } else {
              BCrypt.compare(
                password,
                HighSaleList.password,
                function (error, result) {
                  if (result === true) {
                    return callback(null, null, 200, null, HighSaleList);
                  } else {
