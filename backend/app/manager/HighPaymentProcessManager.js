...
const HighPaymentProcess = require("../models/HighPaymentProcess");
...
module.exports = {
    ...
    try {
      ...
      HighPaymentProcess.findOne({
        ...
        if (result) {
          ...
          return callback(1, "invalid_HighPaymentProcess", 403, null, null);
        }
      });
    } catch (error) {
      return callback(1, "get_one_HighPaymentProcess_fail", 400, error, null);
    }
  },
  ...
  getAll: function (
    ...
    try {
      ...
      HighPaymentProcess.findAndCountAll({
        where: where,
  ...
  //             .then((data) => {
  //                 let pages = Math.ceil(data.count / perPage);
  //                 let HighPaymentProcesss = data.rows;
  //                 let output = {
  //                     data: HighPaymentProcesss,
  //                     pages: {
  //                         current: page,
  //                         prev: page - 1,
  //                         hasPrev: false,
  ...
  update: function (
    ...
    try {
      ...
      HighPaymentProcess.update(queryObj, { where: where })
        .then((result) => {
          "use strict";
  ...
  delete: function (accessUserId, accessUserType, id, callback) {
    try {
      ...
      HighPaymentProcess.findOne({ where: where })
        .then((HighPaymentProcess) => {
          "use strict";
          if (HighPaymentProcess && HighPaymentProcess.deleted === Constant.DELETED.YES) {
            HighPaymentProcess.destroy({ where: where })
              .then((result) => {
                return callback(null, null, 200, null);
              })
              .catch(function (error) {
                return callback(1, "remove_HighPaymentProcess_fail", 420, error);
              });
          } else {
            HighPaymentProcess.update(queryObj, { where: where })
              .then((result) => {
              ...
              .catch(function (error) {
                return callback(1, "update_HighPaymentProcess_fail", 420, error);
              });
          }
        })
        .catch(function (error) {
          "use strict";
          return callback(1, "find_one_HighPaymentProcess_fail", 400, error, null);
        });
    } catch (error) {
      return callback(1, "delete_HighPaymentProcess_fail", 400, error);
    }
  },
  ...
  authenticate: function (loginName, password, callback) {
    try {
      ...
      HighPaymentProcess.findOne({
        ...
        .then((HighPaymentProcess) => {
          "use strict";
          if (HighPaymentProcess) {
            if (HighPaymentProcess.activated === "false") {
              return callback(1, "unactivated_user", 404, null, null);
            } else {
              BCrypt.compare(
                password,
                HighPaymentProcess.password,
                function (error, result) {
                  if (result === true) {
                    return callback(null, null, 200, null, HighPaymentProcess);
                  } else {
  ...
  createByAdmin: function (
    ...
    try {
      ...
      HighPaymentProcess.create(queryObj)
        .then((result) => {
          "use strict";
          return callback(null, null, 200, null, result);
        })
