...
module.exports = {
    ...
    try {
      ...
      PaymentMethod.findOne({
        ...
        if (result) {
          ...
          return callback(1, "invalid_PaymentMethod", 403, null, null);
        }
      });
    } catch (error) {
      return callback(1, "get_one_PaymentMethod_fail", 400, error, null);
    }
  },
  ...
  update: function (
    ...
    try {
      ...
      queryObj.total_of_payment_time = updateData.total_of_payment_time;
      queryObj.method_name = updateData.method_name;
      queryObj.percent_discount = updateData.percent_discount;
      ...
      PaymentMethod.update(queryObj, { where: where })
        .then((result) => {
          "use strict";
  ...
  delete: function (accessUserId, accessUserType, id, callback) {
    try {
      ...
      PaymentMethod.findOne({ where: where })
        .then((PaymentMethod) => {
          "use strict";
          if (PaymentMethod && PaymentMethod.deleted === Constant.DELETED.YES) {
            PaymentMethod.destroy({ where: where })
              .then((result) => {
                return callback(null, null, 200, null);
              })
              .catch(function (error) {
                return callback(1, "remove_PaymentMethod_fail", 420, error);
              });
          } else {
            PaymentMethod.update(queryObj, { where: where })
              .then((result) => {
              ...
              .catch(function (error) {
                return callback(1, "update_PaymentMethod_fail", 420, error);
              });
          }
        })
        .catch(function (error) {
          "use strict";
          return callback(1, "find_one_PaymentMethod_fail", 400, error, null);
        });
    } catch (error) {
      return callback(1, "delete_PaymentMethod_fail", 400, error);
    }
  },
